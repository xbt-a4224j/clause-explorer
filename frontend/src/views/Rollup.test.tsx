...
import { Rollup } from '@semantic-explorer-base/ui'
...
const ROLLUP: RollupResponse = {
  ...
  refused: false,
  refusal: null,
  answered_subject_count: 2,
  absent_subject_count: 1,
  scope_note:
...
describe('the count-vs-percentage rule', () => {
  it('renders "6 of 8" and never a percentage below the threshold', async () => {
    render(<Rollup selection={EIGHT} strings={STRINGS} scopeFallback="x" />)
...
describe('min_n refusal', () => {
  ...
  it('renders a distinct refusal state stating the actual n and the threshold', async () => {
    ...
    render(<Rollup selection={['contract_1', 'contract_2']} strings={STRINGS} scopeFallback="x" />)
...
describe('designed states', () => {
  it('asks for a selection rather than rolling up the whole corpus', () => {
    render(<Rollup selection={[]} strings={STRINGS} scopeFallback="x" />)
